...
/*
StyleFix Canary Fixture Builder v1.0.7

Builder-only correction release. The StyleFix scanner remains v1.0.6.

Changes from v1.0.6:
- startup capability probe for builder mutation/read-back primitives;
- Story-to-Text normalization before applyCharacterStyle;
- explicit Endnote.insertTextInEndnote path with post-insert reacquisition;
- exact literal-span verification checks every character in the planted span.

The builder does not import or reuse StyleFix scanner traversal code.
*/

(function () {
    var VERSION = "1.0.7";
    var EXPECTED_SCANNER = "1.0.6";
    ...
    function createStyles() {
        ...
        styles.F01.pointSize = 12; styles.F01.tracking = 10;
        styles.F02.pointSize = 13; styles.F02.tracking = 20;
        styles.F03.pointSize = 14; styles.F03.tracking = 11;
        styles.F04.pointSize = 15; styles.F04.tracking = 30;
    }
    ...
    function probeBuilderCapabilities() {
        var pd = null, tf, story, text, probeStyle, ip, en;
        var failures = [];
        function need(ok, label) { if (!ok) { failures.push(label); } }

        try {
            pd = app.documents.add(true);
            pd.documentPreferences.facingPages = false;
            tf = pd.pages.item(0).textFrames.add(pd.layers.item(0));
            tf.geometricBounds = [36,36,72,260];
            tf.contents = "StyleFix builder probe";

            story = tf.parentStory;
            need(story && story.isValid === true, "Story object");
            need(story.texts && story.texts.length > 0, "Story.texts");

            text = textObject(story);
            need(text !== null, "Story-to-Text resolution");
            need(text !== null && typeof text.applyCharacterStyle === "function", "Text.applyCharacterStyle");
            need(text !== null && text.characters && typeof text.characters.itemByRange === "function",
                "Text.characters.itemByRange");

            probeStyle = pd.characterStyles.add({name:"StyleFix Builder Probe"});
            need(probeStyle && probeStyle.isValid === true, "CharacterStyles.add");
            need(probeStyle.styleExportTagMaps &&
                typeof probeStyle.styleExportTagMaps.add === "function", "StyleExportTagMaps.add");

            if (text !== null && typeof text.applyCharacterStyle === "function") {
                text.applyCharacterStyle(probeStyle);
                need(sameStyleDirect(text.characters.item(0).appliedCharacterStyle,probeStyle),
                    "Character.appliedCharacterStyle read-back");
            }

            ip = story.insertionPoints.item(-1);
            need(ip && typeof ip.createEndnote === "function", "InsertionPoint.createEndnote");
            if (ip && typeof ip.createEndnote === "function") {
                en = ip.createEndnote();
                need(en && en.isValid === true, "Endnote creation");
                need(en && typeof en.insertTextInEndnote === "function", "Endnote.insertTextInEndnote");
                try { need(en.endnoteTextRange !== undefined, "Endnote.endnoteTextRange"); }
                catch (eRange) { failures.push("Endnote.endnoteTextRange"); }
                try { en.deleteEndnote(); } catch (ignoreDelete) {}
            }
        } catch (eProbe) {
            failures.push(errText(eProbe));
        } finally {
            if (pd !== null) {
                try { pd.close(SaveOptions.NO); } catch (ignoreClose) {}
            }
        }

        if (failures.length > 0) {
            failed++;
            log.push("FAIL\tBUILDER CAPABILITY PROBE\t" + failures.join(" | "));
            return false;
        }
        log.push("PASS\tBUILDER CAPABILITY PROBE\tStory.texts | Text.applyCharacterStyle | " +
            "Text.characters.itemByRange | Endnote.insertTextInEndnote | export tag maps");
        return true;
    }
    ...
    function buildC01() {
        ...
        registerDirect("C01","Ordinary story",function(){return tf.parentStory;},styles.C01,"C01 ordinary story text");
    }
    ...
    function buildC02() {
        ...
        registerDirect("C02","Table cell",function(){return cell.texts.item(0);},styles.C02,"C02 table cell");
    }
    ...
    function buildC03() {
        ...
        registerDirect("C03","Nested table cell",function(){return nestedCell.texts.item(0);},styles.C03,"C03 nested table cell");
    }

    function buildC04() {
        var tf = addFrame(page1,[252,36,324,300],"C04 host text ");
        var fn = tf.parentStory.footnotes.add(LocationOptions.AFTER,tf.parentStory.insertionPoints.item(-1));
        fn.contents = "C04 footnote text";
        applyAll(fn.texts.item(0),styles.C04);
        registerDirect("C04","Footnote text",function(){return fn.texts.item(0);},styles.C04,"C04 footnote text");
    }

    function buildC05() {
        var tf = addFrame(doc.pages.item(1),[36,36,108,300],"C05 host text ");
        var ip = tf.parentStory.insertionPoints.item(-1);
        var en, insertAt, text;
        try { doc.endnoteOptions.frameCreateOption = EndnoteFrameCreate.NEW_PAGE; } catch (ignoreOption) {}

        en = ip.createEndnote();
        insertAt = endnoteInsertPoint(en);
        en.insertTextInEndnote(insertAt,"C05 endnote text");

        text = reacquireEndnoteText(en);
        if (text === null) { throw new Error("Could not reacquire endnote text after insertion."); }
        applyLiteral(text,"C05 endnote text",styles.C05);

        registerDirect("C05","Endnote text",function(){return reacquireEndnoteText(en);},
            styles.C05,"C05 endnote text");
    }
    ...
    function buildC06() {
        ...
        applyLiteral(tf.parentStory,"C06 OVERSET CANARY",styles.C06);
        registerDirect("C06","Overset text",function(){return tf.parentStory;},styles.C06,"C06 OVERSET CANARY");
    }
    ...
    function buildC07() {
        ...
        registerDirect("C07","Pasteboard text frame",function(){return tf.parentStory;},styles.C07,"C07 pasteboard text frame");
    }

    function buildC08() {
        var mp = doc.masterSpreads.item(0).pages.item(0);
        var tf = addFrame(mp,[36,36,72,300],"C08 parent page text frame");
        applyAll(tf.parentStory,styles.C08);
        registerDirect("C08","Parent/master page text frame",function(){return tf.parentStory;},styles.C08,"C08 parent page text frame");
    }
    ...
    function buildC09() {
        ...
        registerDirect("C09","Anchored inline text frame",function(){return child.parentStory;},styles.C09,"C09 anchored inline frame");
    }
    ...
    function buildC10() {
        ...
        registerDirect("C10","Grouped text frame",function(){return tf.parentStory;},styles.C10,"C10 grouped text frame");
    }
    ...
    function buildC11() {
        ...
        layer.visible = false;
        registerDirect("C11","Hidden-layer text frame",function(){return tf.parentStory;},styles.C11,"C11 hidden layer text");
    }
    ...
    function buildC12() {
        ...
        layer.locked = true;
        registerDirect("C12","Locked-layer text frame",function(){return tf.parentStory;},styles.C12,"C12 locked layer text");
    }

    function buildC13() {
        var a = addFrame(doc.pages.item(2),[150,36,310,300],"");
        var b = addFrame(doc.pages.item(3),[36,36,196,300],"");
        var c = addFrame(doc.pages.item(4),[36,36,196,300],"");
        var content = "", i;
        a.nextTextFrame = b;
        b.nextTextFrame = c;
        for (i = 0; i < 100; i++) {
        ...
        registerDirect("C13","Threaded three-page story",function(){return a.parentStory;},styles.C13,"C13 threaded story line");
    }
    ...
    function buildC14() {
        ...
        registerDirect("C14","Table on parent/master page",function(){return cell.texts.item(0);},styles.C14,"C14 table on parent page");
    }

    function buildD01() {
        var ps = doc.paragraphStyles.add({name:"Canary D01 Bullet"});
        ps.bulletsCharacterStyle = styles.D01;
        registerDependency("D01","Bullet character style",
            function(){return ps.bulletsCharacterStyle;},"bulletsCharacterStyle",styles.D01);
    }

    function buildD02() {
        var ps = doc.paragraphStyles.add({name:"Canary D02 GREP"});
        var ng = ps.nestedGrepStyles.add({appliedCharacterStyle:styles.D02,grepExpression:"CANARY_D02"});
        registerDependency("D02","Nested GREP style",
            function(){return ng.appliedCharacterStyle;},"appliedCharacterStyle",styles.D02);
    }
    ...
    function buildD03() {
        ...
        registerDependency("D03","TOC page-number style",
            function(){return entry.pageNumberStyle;},"pageNumberStyle",styles.D03);
    }

    function buildD04() {
        var fmt = doc.crossReferenceFormats.add("Canary D04 Format");
        var bb = fmt.buildingBlocks.add(BuildingBlockTypes.CUSTOM_STRING_BUILDING_BLOCK,styles.D04,"D04");
        registerControl("D04","Dependency","Cross-reference building block",function(){
            return sameStyleDirect(buildingBlockStyle(bb),styles.D04);
        },function(){return "buildingBlockStyle=" + directStyleName(buildingBlockStyle(bb));});
    }

    function buildD05() {
        var tf = addFrame(doc.pages.item(5),[36,36,72,300],"D05 hyperlink source text");
        var source = doc.hyperlinkTextSources.add(tf.parentStory.texts.item(0),
            {appliedCharacterStyle:styles.D05});
        registerDependency("D05","Hyperlink text source",
            function(){return source.appliedCharacterStyle;},"appliedCharacterStyle",styles.D05);
    }

    function buildD06() {
        doc.footnoteOptions.footnoteMarkerStyle = styles.D06;
        registerDependency("D06","Footnote marker style",
            function(){return doc.footnoteOptions.footnoteMarkerStyle;},"footnoteMarkerStyle",styles.D06);
    }

    function buildD07() {
        var child = doc.characterStyles.add({name:"Canary Child Based On D07",basedOn:styles.D07,pointSize:9});
        registerDependency("D07","Character-style basedOn",
            function(){return child.basedOn;},"basedOn",styles.D07);
    }

    function buildE01() {
        registerControl("E01","Formatting/export","Empty shell, no export map",
            function(){return exportMapCountDirect(styles.E01) === 0;},
            function(){return "exportMaps=" + exportMapCountDirect(styles.E01);});
    }

    function buildE02() {
        styles.E02.styleExportTagMaps.add("EPUB","span","canary-e02","");
        registerControl("E02","Formatting/export","Custom EPUB tag/class",
            function(){return exportMapCountDirect(styles.E02) >= 1;},
            function(){return "exportMaps=" + exportMapCountDirect(styles.E02);});
    }
    ...
    function buildF02() {
        ...
        registerControl("F02","Fingerprint","Used single canonical match",function(){
            return verifyLiteralStyled(tf.parentStory,styles.F02,"F02 used single match") &&
                sameNumber(styles.F02.pointSize,canonical.F02.pointSize) &&
                sameNumber(styles.F02.tracking,canonical.F02.tracking);
        },function(){return directTextEvidence(tf.parentStory,styles.F02,"F02 used single match");});
    }
    ...
    function buildF03() {
        ...
        registerControl("F03","Fingerprint","Tracking-only mismatch",function(){
            return verifyLiteralStyled(tf.parentStory,styles.F03,"F03 tracking mismatch") &&
                !sameNumber(styles.F03.tracking,canonical.F03.tracking);
        },function(){
            return directTextEvidence(tf.parentStory,styles.F03,"F03 tracking mismatch") +
                ";candidateTracking=" + styles.F03.tracking +
    ...
    function buildF04() {
        ...
        registerControl("F04","Fingerprint","Two canonical matches",function(){
            return verifyLiteralStyled(tf.parentStory,styles.F04,"F04 duplicate canonical match") &&
            ...
            return directTextEvidence(tf.parentStory,styles.F04,"F04 duplicate canonical match") +
                ";twoCanonicalMatches=" + canonical.F04A.name + "|" + canonical.F04B.name;
        });
    }
    ...
    function buildP01Record() {
        ...
        },function(){return "paragraphStyle=" + styles.P01.name;});
    }

    function registerDirect(id,context,provider,style,literal) {
        registerControl(id,"Direct use",context,function(){
            return verifyLiteralStyled(provider(),style,literal);
        },function(){return directTextEvidence(provider(),style,literal);});
    }

    function registerDependency(id,context,provider,label,style) {
        registerControl(id,"Dependency",context,function(){
            return sameStyleDirect(provider(),style);
        },function(){return label + "=" + directStyleName(provider());});
    }

    function registerControl(id,cls,context,verifyFn,evidenceFn) {
        controls.push({
            id:id,controlClass:cls,context:context,
            verify:verifyFn,evidence:evidenceFn,verified:false,evidenceText:""
        });
    }
    ...
    function verifyRegisteredControls() {
        var i, c, ok, evidence;
        var classCounts = {
            "Direct use":0,"Dependency":0,"Formatting/export":0,"Fingerprint":0,"Paragraph census":0
        };
        ...
        for (i = 0; i < controls.length; i++) {
            ...
            if (ok && classCounts[c.controlClass] !== undefined) { classCounts[c.controlClass]++; }
            log.push((ok ? "PASS" : "FAIL") + "\tVERIFY " + c.id + "\t" + c.context + "\t" + evidence);
            if (!ok) { failed++; }
        }

        requireCount("Registered controls",controls.length,28);
    ...
    function textObject(obj) {
        var text;
        if (!obj || obj.isValid !== true) { return null; }
        try {
            if (typeof obj.applyCharacterStyle === "function") { return obj; }
        } catch (ignoreDirect) {}
        try {
            if (obj.texts && obj.texts.length > 0) {
                text = obj.texts.item(0);
                if (text && text.isValid === true &&
                    typeof text.applyCharacterStyle === "function") { return text; }
            }
        } catch (ignoreTexts) {}
        return null;
    }
    ...
    function applyAll(obj,style) {
        var text = textObject(obj);
        if (text === null) {
            throw new Error("Could not resolve a Text object exposing applyCharacterStyle.");
        }
        text.applyCharacterStyle(style);
        return text;
    }

    function applyLiteral(obj,literal,style) {
        var contents, idx, chars, range;
        if (!obj || obj.isValid !== true) { throw new Error("Invalid text container."); }
        contents = String(obj.contents);
        idx = contents.indexOf(literal);
        if (idx < 0) { throw new Error("Literal not found: " + literal); }
        chars = obj.characters;
        if (!chars || idx + literal.length > chars.length) {
            throw new Error("Literal character span is outside the container.");
        }
        range = chars.itemByRange(idx,idx + literal.length - 1);
        range.applyCharacterStyle(style);
    }

    function verifyLiteralStyled(obj,style,literal) {
        var contents, idx, chars, i, ref;
        if (!obj || obj.isValid !== true) { return false; }
        try { contents = String(obj.contents); } catch (eContents) { return false; }
        idx = contents.indexOf(literal);
        if (idx < 0) { return false; }
        try {
            chars = obj.characters;
            if (!chars || idx + literal.length > chars.length) { return false; }
            for (i = 0; i < literal.length; i++) {
                ref = chars.item(idx + i).appliedCharacterStyle;
                if (!sameStyleDirect(ref,style)) { return false; }
            }
        } catch (eChars) { return false; }
        return true;
    }

    function directTextEvidence(obj,style,literal) {
        var contents = "", idx = -1, chars = null, checked = 0, matching = 0, i;
        try { contents = String(obj.contents); idx = contents.indexOf(literal); } catch (ignoreContents) {}
        try {
            chars = obj.characters;
            if (idx >= 0 && chars && idx + literal.length <= chars.length) {
                checked = literal.length;
                for (i = 0; i < literal.length; i++) {
                    if (sameStyleDirect(chars.item(idx + i).appliedCharacterStyle,style)) { matching++; }
                }
            }
        } catch (ignoreChars) {}
        return "literal=" + (idx >= 0 ? "YES" : "NO") +
            ";spanChars=" + checked +
            ";matchingStyleChars=" + matching +
            ";expected=" + directStyleName(style);
    }

    function endnoteInsertPoint(en) {
        var range, ips;
        try {
            range = en.endnoteTextRange;
            if (range && range.isValid === true && range.insertionPoints) {
                ips = range.insertionPoints;
                if (ips.length > 2) { return ips.item(1); }
                if (ips.length > 0) { return ips.item(0); }
            }
        } catch (ignoreRange) {}
        try {
            ips = en.insertionPoints;
            if (ips.length > 2) { return ips.item(1); }
            if (ips.length > 0) { return ips.item(0); }
        } catch (ignoreEndnoteIps) {}
        throw new Error("Could not find an insertion point inside the endnote text range.");
    }

    function reacquireEndnoteText(en) {
        var text, resolved;
        if (!en || en.isValid !== true) { return null; }
        try {
            text = en.texts.item(0);
            try {
                resolved = text.getElements();
                if (resolved && resolved.length > 0) { text = resolved[0]; }
            } catch (ignoreResolve) {}
            if (text && text.isValid === true) { return text; }
        } catch (ignoreText) {}
        return null;
    }

    function buildingBlockStyle(bb) {
        var ref = null;
        try { ref = bb.appliedCharacterStyle; } catch (ignore1) {}
        if (ref === null) { try { ref = bb.appliedStyle; } catch (ignore2) {} }
        return ref;
    }
    ...
    function directStyleName(style) {
        if (!style) { return ""; }
        try { return String(style.name); } catch (ignore) { return ""; }
    }

    function exportMapCountDirect(style) {
        try { return style.styleExportTagMaps.length; } catch (ignore) { return -1; }
    }
    ...
    function countStoryFrames(story) {
        try { return story.textContainers.length; } catch (ignore) { return 0; }
    }
    ...
    function finalize() {
        ...
        if (logFile.open("w")) {
            ...
            logFile.writeln("Expected StyleFix scanner: " + EXPECTED_SCANNER);
        ...
        if (failed === 0) {
            alert("StyleFix canary fixture built and verified successfully.\n\n" +
                "Builder: v" + VERSION + "\nExpected scanner: v" + EXPECTED_SCANNER +
                "\n\nINDD:\n" + target.fsName +
                "\n\nIDML:\n" + idml.fsName +
                "\n\nBuilder census:\n" + censusFile.fsName +
                "\n\nBuild log:\n" + logFile.fsName +
                "\n\nNext: run VerifyCanaryIDML.py, then run StyleFix v" +
                EXPECTED_SCANNER + " against the INDD.");
        } else {
    ...
    function writeCensus(file) {
        ...
        file.write("\uFEFF");
        file.writeln(csv([
            "Fixture Version","Expected Scanner Version","Fixture Schema","Fixture ID",
            "ID","Control Class","Construction Context","Verified","Evidence"
        ]));
        for (i = 0; i < controls.length; i++) {
            c = controls[i];
            file.writeln(csv([
                VERSION,EXPECTED_SCANNER,FIXTURE_SCHEMA,FIXTURE_ID,
    ...
    function safeApp(prop) {
        try { return String(app[prop]); } catch (ignore) { return ""; }
    }

    function safeOs() {
        try { return String($.os); } catch (ignore) { return ""; }
    }
